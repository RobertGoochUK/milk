CREATE DEFINER=`masteruser`@`%` PROCEDURE `sp_VTMtoVMP`(
...
vmp.vmpid, 
vmp.name,
calc_rank(IN_dose_qty,convert_units(vpi.strnt_nmrtr_val,vpi.strnt_nmrtr_uomcd,IN_dose_uom_cd),vpi.strnt_dnmtr_val,vmp.udfs) AS qty,
vmp.udfs_dose_uomcd,
vpi.strnt_dnmtr_uomcd,
vmp.pres_statcd,
calc_rank3(IN_dose_qty,convert_units(vpi.strnt_nmrtr_val,vpi.strnt_nmrtr_uomcd,IN_dose_uom_cd),vpi.strnt_dnmtr_val,vmp.udfs,vmpform.formid) AS rank
FROM vtm 
